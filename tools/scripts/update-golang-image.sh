...
used_by=($(git grep -l docker.io/library/golang ${PROJECT_ROOT_PATH}/image/*/Dockerfile))
...
for i in "${used_by[@]}" ; do
    ...
    sed "s|docker\.io/library/golang:[0-9][0-9]*\.[0-9][0-9]*\(\.[0-9][0-9]*\)\?|${image}|" "${i}" > "${i}.sedtmp" && mv ${i}.sedtmp ${i}
done
